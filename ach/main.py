...
def main_page():
    ...
    # Tab 2: Random Movies
    cursor.execute("""
        SELECT movieId, title, overview, voteAverage, coverUrl
        FROM Movies 
    ...
    cursor.execute("""
        SELECT YEAR(releaseDate) AS releaseYear, m.movieId, m.title, m.overview, m.voteAverage, m.coverUrl, COUNT(c.commentId) AS commentCount
        FROM Movies m
        JOIN Comments c ON m.movieId = c.movieId
        WHERE c.timeStamp >= %s
        GROUP BY m.movieId, m.title, m.overview, m.voteAverage, m.coverUrl
    ...
    cursor.execute("""
        SELECT m.movieId, m.title, m.overview, m.voteAverage, m.coverUrl, COUNT(c.commentId) AS totalComments
        FROM Movies m
...
def search():
    ...
    cursor.execute("""
        SELECT DISTINCT Movies.movieId, Movies.title, Movies.overview, Movies.coverUrl
        FROM Movies
